...
def remove_underscore_before_word(string, word):
    ...
    if index != -1:
        string = (
            string[: index - 1]
            + string[index - 1 : index].replace("_", " ")
            + string[index:]
        )
    return string
...
def process_name_change(energy_processes_transformed):
    ...
    for elem in energy_processes_transformed:
        ...
        if elem[0:2] == "PP":
            ...
            if elem[-6:] == "w/ CCS":
                new_elem = (
                    new_elem + "|" + change_nomnclature_element(elem[3:-6]) + elem[-6:]
                )
            elif elem[-7:] == "w/o CCS":
                new_elem = (
                    new_elem + "|" + change_nomnclature_element(elem[3:-7]) + elem[-7:]
                )
            else:
...
def main():
    # ### Country Dict
    file_path = (
        "./Variable_Reference/reference.xlsx"  # Update this with the correct path
    )
    ...
    country_Wiliam_dict["EU27"] = (
        "European Union (27 member countries)"  # 27 countries only different with the previous versions of
    )
    ...
    country_Wiliam_dict["USMCA"] = (
        "United States, Mexico and Canada"  # not correponding in an according manner, there is mexico also.
    )
    ...
    age_subscript_list = [
        "c" + str(number_list[k][0]) + "c" + str(number_list[k][1])
    ...
    new_age_subscript_list = [
        "Age " + str(number_list[k][0]) + "-" + str(number_list[k][1])
        for k in range(len(number_list))
    ]
    ...
    primary_energy_list = [
        primary_energy.replace(" ", "_") for primary_energy in primary_energy_raw_list
    ]
    ...
    energy_primary_dict = dict(
        zip(primary_energy_list, transformed_primary_energy_list)
    )
    ...
    transformation_dict = dict(
        zip(transformation_output_old, transformation_output_new)
    )
