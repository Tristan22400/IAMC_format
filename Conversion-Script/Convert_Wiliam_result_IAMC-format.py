    ...
    for row_index, row in emission_rows[["Variable", "Unit"]].iterrows():
        ...
        for string in GHGs_to_check:
            if string in row["Variable"]:
                ...
                row["Variable"] = (
                    row["Variable"][:index_emission]
                    + "|"
                    + string
                    + row["Variable"][index_emission : index_gas - 1]
                    + row["Variable"][index_gas + len(string) :]
                )
                ghg = string
                ghg_bool = True
        ...
        if row["Unit"] == "Mt/yr" or row["Unit"] == "Gt/yr":
            if ghg_bool:
                row["Unit"] = (
                    row["Unit"][:index_year] + " " + ghg + row["Unit"][index_year:]
                )
            else:
                row["Unit"] = (
                    row["Unit"][:index_year] + " " + string + row["Unit"][index_year:]
                )
...
def move_per_capita(s):
    ...
    if specific_word in s:
        ...
        s = (
            s[: index_emission - 1]
            + s[index_emission + len(specific_word) :]
            + "|"
            + specific_word
        )
...
def remove_string_between_pipe(input_string):
    ...
    result = remove_characters_between_indices(
        input_string, second_occurrence, third_occurrence
    )
...
def main():
    ...
    scenario_variable_df["Variable"] = scenario_variable_df["Variable"].replace(
    ...
    scenario_variable_df = scenario_variable_df[
        scenario_variable_df["Variable"].isin(variable_required_list)
    ]

    # ### Check of the variable included in the file

    variables_used_list = scenario_variable_df["Variable"].to_list()
    variables_required_not_in = list(
        set(variable_required_list) - set(variables_used_list)
    )
    print(
        "The missing variable in the export dataset are the following:",
        variables_required_not_in,
    )
    ...
    scenario_variable_df = scenario_variable_df.apply(
        aggregate_variable_name,
        args=(counter, country_dict, energy_dict, rest_dict),
        axis=1,
    )
    ...
    scenario_variable_df["Variable"] = scenario_variable_df["Variable"].apply(
        transform_string, args=(upper_word_list,)
    )
    scenario_variable_df
    ...
    scenario_variable_df["Region"] = scenario_variable_df["Region"].replace(
        country_Wiliam_dict
    )
    scenario_variable_df
    ...
    emission_rows = scenario_variable_df[
        scenario_variable_df["Variable"].str.contains("Emission")
    ]
    ...
    variable_list, unit_list = emission_correction(
        emission_rows, GHGs_to_check, HFC_list
    )
    ...
    energy_rows = scenario_variable_df[
        scenario_variable_df["Variable"].str.contains("Per Capita")
    ]
    ...
    scenario_variable_df["Variable"] = scenario_variable_df["Variable"].replace(
        energy_per_capita_dict
    )

    # Change Secondary Energy Naming
    secondary_energy_rows = scenario_variable_df[
        scenario_variable_df["Variable"].str.contains("Secondary Energy")
    ]
    secondary_energy_rows["Variable"].iloc[0]

    # Change Secondary Energy Naming
    # Select the rows for Primary Energy Price
    price_energy_rows = scenario_variable_df[
        scenario_variable_df["Variable"].str.contains("Price")
    ]
    price_energy_rows = price_energy_rows[
        price_energy_rows["Variable"].str.contains("Primary Energy")
    ]

    price_energy_rows["Variable"] = price_energy_rows["Variable"].apply(
        remove_string_between_pipe
    )
    ...
    scenario_variable_df.drop_duplicates(
        subset=["Region", "Variable", "Unit"], inplace=True
    )
